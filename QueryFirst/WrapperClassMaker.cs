...
";
        ...
        public virtual string MakeExecuteWithoutConn(CodeGenerationContext ctx)
        {
            ...
            code.AppendLine("return Execute(" + GetCallingArgs(ctx) + ").ToList();");
            code.AppendLine("}");
            code.AppendLine("}");
        ...
        public virtual string MakeExecuteWithConn(CodeGenerationContext ctx)
        {
            ...
            {
                code.AppendLine("AddAParameter(cmd, \"" + qp.DbType + "\", \"" + qp.DbName + "\", " + GetCallingArgPrefix(ctx) + qp.CSName + ", " + qp.Length + ");");
            }
        ...
        public virtual string MakeGetOneWithoutConn(CodeGenerationContext ctx)
        {
            ...
            code.AppendLine("{");
            code.AppendLine("conn.Open();");
            code.AppendLine("return GetOne(" + GetCallingArgs(ctx) + ");");
            code.AppendLine("}");
            code.AppendLine("}");
            return code.ToString();
        ...
        public virtual string MakeGetOneWithConn(CodeGenerationContext ctx)
        {
            ...
            code.AppendLine("{");
            code.AppendLine("var all = Execute(" + GetCallingArgs(ctx) + ");");
        ...
        public virtual string MakeExecuteScalarWithoutConn(CodeGenerationContext ctx)
        {
            ...
            code.AppendLine("{");
            code.AppendLine("conn.Open();");
            code.AppendLine("return ExecuteScalar(" + GetCallingArgs(ctx) + ");");
            code.AppendLine("}");
            code.AppendLine("}");
            return code.ToString();
        }
        ...
        public virtual string MakeExecuteNonQueryWithoutConn(CodeGenerationContext ctx)
        {
            ...
            code.AppendLine("{");
            code.AppendLine("conn.Open();");
            code.AppendLine("return ExecuteNonQuery(" + GetCallingArgs(ctx) + ");");
            code.AppendLine("}");
            code.AppendLine("}");
            return code.ToString();
        }
        ...
        public string MakeInterface(CodeGenerationContext ctx, bool requireConnectionParameter = false)
        {
            char[] spaceComma = new char[] { ',', ' ' };
            StringBuilder code = new StringBuilder();
